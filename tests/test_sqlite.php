...
class TestMysql{
    ...
    function connect() {
        $this->db = DB::initSqlite('sqlite::memory:');
        //$this->db = FiveDB::initSqlite('sqlite:/tmp/sqlite-db.sq3');
    }
    
    function showTables() {
        $arr = $this->db->fetchAll('select * from sqlite_master where type="table"', []);
        echo "show tables:\n";
        print_r($arr);
    }
  ...
  `k_host_conf` varchar(32) NOT NULL ,
  `test` int NOT NULL,
  `update_time` int NOT NULL ,
  `str_time` text NOT NULL DEFAULT ""
) ';
    ...
    function update() {
        ...
        echo "-- 修改=f+?+?\n";
        $this->db->update($tb, $arr_where, ['test' => ['test+?+?', [1,3]]]);
        print_r($this->db->getTableWhereRow($tb,  $arr_where));
        
        echo "-- 修改=f1\n";
        $this->db->update($tb, $arr_where, ['test' => ['update_time', []]]);
        print_r($this->db->getTableWhereRow($tb,  $arr_where));
    ...
    function delete() {
        ...
        echo "-- 清空表\n";
        printf("count:%d\n", $this->db->getTableWhereCount($tb, []));
        //$this->db->exec('delete from aa_test', []);
        printf("count:%d\n", $this->db->getTableWhereCount($tb, []));
    }
}
